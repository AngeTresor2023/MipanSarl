...
export async function POST(req: Request) {
  const { supabase, error } = await requireAdmin();
  if (error) return error;
  const { product_id, product_title, quantity, unit_price, note, sold_at } = await req.json();
  if (!product_title || !quantity || unit_price === undefined)
  ...
  const { data, error: err } = await supabase!.from("local_sales").insert({
...
export async function DELETE(req: Request) {
  ...
  const { error: err } = await supabase!.from("local_sales").delete().eq("id", id);
  if (err) return NextResponse.json({ error: err.message }, { status: 500 });
